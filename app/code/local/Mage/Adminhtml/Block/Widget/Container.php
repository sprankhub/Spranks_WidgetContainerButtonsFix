...
class Mage_Adminhtml_Block_Widget_Container extends Mage_Adminhtml_Block_Template
{
    ...
    public function getButtonsHtml($area = null)
    {
        ...
        foreach ($this->_buttons as $level => $buttons) {
            $_buttons = array();
            foreach ($buttons as $id => $data) {
                if (empty($_buttons[$data['sort_order']])) {
                    $_buttons[$data['sort_order']] = array();
                }
                $nextIndex = count($_buttons[$data['sort_order']]);
                $_buttons[$data['sort_order']][$nextIndex]['id'] = $id;
                $_buttons[$data['sort_order']][$nextIndex]['data'] = $data;
            }
            ksort($_buttons);
            foreach ($_buttons as $buttonArray) {
